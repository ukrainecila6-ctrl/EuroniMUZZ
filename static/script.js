...
if(trackList){
    tracks = Array.from(trackList.children);
    tracks.forEach((li, i)=> li.addEventListener('click', ()=>{currentIndex=i; playTrack();}));
}
...
if(recTrackList){
    recTracks = Array.from(recTrackList.children);
    recTracks.forEach((li,i)=>li.addEventListener('click', ()=>{recIndex=i; playRecTrack();}));
}

function playRecTrack(){
    recAudio.src = recTracks[recIndex].dataset.src;
    recAudio.play();
    recIsPlaying = true;
    recPlayPauseBtn.textContent='⏸️';
    recPlayer.classList.remove('hidden');
}

if(recPlayPauseBtn) recPlayPauseBtn.addEventListener('click', ()=>{
    if(recIsPlaying){recAudio.pause(); recPlayPauseBtn.textContent='▶️';}
    else {recAudio.play(); recPlayPauseBtn.textContent='⏸️';}
...
recAudio.addEventListener('timeupdate', ()=>{
    if(!recAudio.duration) return;
    const percent = (recAudio.currentTime/recAudio.duration)*100;
    if(recProgress) recProgress.value = percent;
    if(recCurrentTimeEl) recCurrentTimeEl.textContent = formatTime(recAudio.currentTime);
    if(recDurationEl) recDurationEl.textContent = formatTime(recAudio.duration);
});
